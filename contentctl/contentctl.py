...
# from contentctl.objects.config import init, validate, build,  new, deploy_acs, deploy_rest
...
# def build_func(args, config:Union[Config,None]=None) -> DirectorOutputDto:
#     if config == None:
#         config = start(args)
#     if args.type == "app":
...
#     app = App.appFromConfig(config=config, 
#                             built_app_path=pathlib.Path(config.build.path_root)/f"{config.build.name}-{config.build.version}.tar.gz")
#     app.hardcoded_path = app.get_app_source(
#         config,
