...
class BasicBuilder():
    ...
    def setObject(self, path: pathlib.Path, type: SecurityContentType) -> None:
        ...
        if type == SecurityContentType.deployments:
            ...
            try:
                self.security_content_obj = Deployment.model_validate(yml_dict)
            ...
            try:
                self.security_content_obj = Macro.model_validate(yml_dict)
            except ValidationError as e:
                print('Validation Error for file ' + str(path))
                print(e)
                sys.exit(1)
            ...
            try:
                self.security_content_obj = Lookup.model_validate(yml_dict)
            except ValidationError as e:
                print('Validation Error for file ' + str(path))
                print(e)
                sys.exit(1)
            ...
            try:
                self.security_content_obj = UnitTest.model_validate(yml_dict)
            except ValidationError as e:
                print('Validation Error for file ' + str(path))
                print(e)
                sys.exit(1)
