...
class Inspect:
    def __init__(self, args):
        ...
        if args.package_path is None:
            self.package_path = os.path.join(args.template_answers['build_path'], f"{args.template_answers['APP_NAME']}-{args.template_answers['APP_VERSION']}.tar.gz")
        else:
        ...
        try:
            print(f"Inspecting {self.package_path}")
