...
class TestingStats:
    ...
    def getAvgTimePerDetection(self) -> str:
        ...
        time_per_test_rounded = time_per_test - datetime.timedelta(
            microseconds=time_per_test.microseconds
        )
    ...
    def getElapsedTestingTime(self) -> datetime.timedelta:
        if self.testing_start_time is None:
            raise (
                Exception(
                    "Cannot get elapsed time for testing - testing has not begun yet"
                )
            )
    ...
    def setInstanceState(self, instanceState: InstanceState):
        ...
        if (
            self.instance_state == InstanceState.starting
            and instanceState == InstanceState.running
        ):
            self.testing_start_time = datetime.datetime.now()
        elif self.instance_state == InstanceState.running and instanceState in [
            InstanceState.error,
        ...
        if (
            self.instance_state != InstanceState.stopped
            and instanceState == InstanceState.stopped
        ):
...
class SplunkInstance:
    ...
    def get_service(self):
        try:
            ...
            raise (
                Exception(
                    f"Unable to connect to Splunk instance at [{self.config.test_instance_address}]: {str(e)}"
                )
            )
        return service
    ...
    def test_detection(self, detection: Detection, attack_data_root_folder) -> bool:
        ...
        for test in detection.test.tests:
            if test.result is None:
                ...
                at_least_one_test_missing_observables |= (
                    len(test.result.missing_observables) > 0
                )
    ...
    def hec_raw_replay(
        ...
        host: Union[str, None] = DEFAULT_EVENT_HOST,
        use_https: bool = True,
        verify_ssl=False,
        path: str = "services/collector/raw",
        ...
        if self.tokenString.startswith("Splunk "):
            ...
            headers = {
                "Authorization": f"Splunk {self.tokenString}"
            }  # token must begin with 'Splunk
        ...
        if (
            self.config.test_instance_address.lower().startswith("http://")
            and use_https is True
        ):
            raise (
                Exception(
                    f"URL {self.config.test_instance_address} begins with http://, but use_http is {use_https}. "
                    "Unless you have modified the HTTP Event Collector Configuration, it is probably enabled for https only."
                )
            )
        if (
            self.config.test_instance_address.lower().startswith("https://")
            and use_https is False
        ):
            raise (
                Exception(
                    f"URL {self.config.test_instance_address} begins with https://, but use_http is {use_https}. "
                    "Unless you have modified the HTTP Event Collector Configuration, it is probably enabled for https only."
                )
            )
        ...
        if not (
            self.config.test_instance_address.lower().startswith("http://")
            or self.config.test_instance_address.lower().startswith("https://")
        ):
            if use_https:
                prepend = "https://"
            else:
        ...
        if wait_for_ack:
            if self.channel is None:
                raise (
                    Exception(
                        "HEC replay WAIT_FOR_ACK is enabled but CHANNEL is None. Channel must be supplied to wait on ack"
                    )
                )
            ...
            url_with_path = urllib.parse.urljoin(
                f"{base_url}:{self.hec_port}", "services/collector/ack"
            )
    ...
    def replay_attack_data_files(
        self, attackDataObjects: list[UnitTestAttackData], attack_data_folder: str
    ) -> set[str]:
        ...
        for attack_data_file in attackDataObjects:
            try:
                test_indices.add(
                    self.replay_attack_data_file(attack_data_file, attack_data_folder)
                )
            except Exception as e:
                raise (
                    Exception(
                        f"Error replaying attack data file {attack_data_file.data}: {str(e)}"
                    )
                )
        return test_indices

    def replay_attack_data_file(
        self, attackData: UnitTestAttackData, attack_data_folder: str
    ) -> str:
        ...
        descriptor, data_file = mkstemp(
            prefix="ATTACK_DATA_FILE_", dir=attack_data_folder
        )
        if not (
            attackData.data.startswith("https://")
            or attackData.data.startswith("http://")
        ):
            ...
            try:
                ...
                raise (
                    Exception(
                        f"Unable to copy local attack data file {attackData.data} - {str(e)}"
                    )
                )
        ...
        if attackData.update_timestamp:
            ...
            data_manipulation.manipulate_timestamp(
                data_file, attackData.sourcetype, attackData.source
            )

        # Upload the data
        self.hec_raw_replay(
            pathlib.Path(data_file),
            attackData.custom_index,
            attackData.source,
    ...
    def test_detection_search(
        self,
        detection: Detection,
        test: Union[UnitTestTest, UnitTestBaseline],
        FORCE_ALL_TIME=True,
    ) -> UnitTestResult:
        ...
        try:
            ...
            return UnitTestResult(
                job_content=None, missing_observables=[], message=error_message
            )
        ...
        try:
            ...
            for observable in detection.tags.observable:
                ...
                if name is None:
                    raise (
                        Exception(
                            f"Error checking observable {observable} - Name was None"
                        )
                    )
                else:
                    observables_to_check.add(name)
            if len(observables_to_check) > 0:
                observable_splunk_search = (
                    f"{updated_search} | table {' '.join(observables_to_check)}"
                )
                ...
                for res in observable_results_stream:
                    ...
                    for jsonResult in resJson.get("results", []):
                        ...
                        found_observables = set(
                            [
                                observable
                                for observable in observables_to_check
                                if (
                                    observable in jsonResult
                                    and jsonResult[observable] != None
                                    and jsonResult[observable] != ""
                                )
                            ]
                        )
                        if (
                            len(
                                observables_to_check.symmetric_difference(
                                    found_observables
                                )
                            )
                            == 0
                        ...
                        if len(observables_always_found) == 0:
                            ...
                            observables_always_found = found_observables.intersection(
                                observables_always_found
                            )
                ...
                result.update_missing_observables(
                    observables_to_check - observables_always_found
                )
    ...
    def delete_attack_data(
        self, indices: set[str], host: str = DEFAULT_EVENT_HOST
    ) -> bool:
    ...
    def execute_test(
        self, detection: Detection, test: UnitTestTest, attack_data_folder: str
    ) -> bool:

        # replay all of the attack data
        test_indices = self.replay_attack_data_files(
            test.attack_data, attack_data_folder
        )
        ...
        while True:
            ...
            try:
                ...
                if result is None:
                    ...
                    result.add_message(
                        "Did not execute detection because of Baseline Failure."
                    )
                    test.result = result
        ...
        if test.result is None:
            ...
            test.result = UnitTestResult(
                job_content=None,
                message="Failed to produce a test result for unknown reason",
            )

        if self.config.post_test_behavior == PostTestBehavior.always_pause or (
            test.result.success == False
            and self.config.post_test_behavior == PostTestBehavior.pause_on_failure
        ):
    ...
    def execute_baselines(
        self, detection: Detection, unit_test: UnitTestTest
    ) -> Union[UnitTestResult, None]:
        result = None
    ...
    def execute_baseline(
        self, detection: Detection, baseline: UnitTestBaseline
    ) -> UnitTestResult:
    ...
    def configure_hec(self):
        ...
        try:
            res = self.get_service().input(
                path="/servicesNS/nobody/splunk_httpinput/data/inputs/http/http:%2F%2FDETECTION_TESTING_HEC"
            )
            self.tokenString = res.token
            self.print(
                f"HEC Endpoint for [{self.get_name()}] already exists with token [{self.tokenString}].  Using channel [{self.channel}]"
            )
            return
        except Exception as e:
        ...
        try:
            ...
            self.print(
                f"Successfully configured HEC Endpoint for [{self.get_name()}] with channel [{self.channel}] and token [{self.tokenString}]"
            )
            return
    ...
    def teardown(self) -> None:
        ...
        if self.shared_test_objects.force_finish == True:
            self.print(
                f"Finishing due to error or cancellation after testing [{self.testingStats.num_detections_tested}] detections."
            )
        elif self.testingStats.num_detections_tested == 0:
            self.print(f"Did not find any tests and will not start.")
        else:
            self.print(
                f"Finished testing [{self.testingStats.num_detections_tested}] detections."
            )
    ...
    def run(self):
        try:
            ...
            while detection_to_test is not None:
                try:
                    success = self.test_detection(
                        detection_to_test,
                        self.shared_test_objects.attack_data_root_folder,
                    )
                except Exception as e:
                    self.print(
                        f"Unhandled exception while testing detection [{detection_to_test.file_path}]: {str(e)}"
                    )
                    import traceback
    ...
    def wait_for_splunk_ready(
        self, seconds_between_attempts: int = 10, max_wait_time_seconds: int = 0
    ) -> bool:
        ...
        while True:
            ...
            if max_wait_time_seconds == 0:
                raise (
                    Exception(
                        f"Unable to contact Splunk Instance - if this is a Splunk Server ensure that the address, ports, and credentials are correct and that the server is currently running - [{str(e)}]"
                    )
                )
            if timediff.total_seconds() > max_wait_time_seconds:
                raise (
                    Exception(
                        f"Unable to connect to Splunk Instance - Exceeded maximum start time of {max_wait_time_seconds}"
                    )
                )

            time.sleep(seconds_between_attempts)

    def get_number_of_indexed_events(
        self,
        index: str,
        event_host: str = DEFAULT_EVENT_HOST,
        sourcetype: Union[str, None] = None,
    ) -> int:
        ...
        try:
            ...
            if count is None:
                raise Exception(
                    f"Expected the get_number_of_indexed_events search to only return 1 count, but got {num_results} instead."
                )
            ...
            raise Exception(
                "Error trying to get the count while waiting for indexing to complete: %s"
                % (str(e))
            )

    def wait_for_indexing_to_complete(
        self, sourcetype: str, index: str, check_interval_seconds: int = 5
    ) -> bool:
        startTime = timeit.default_timer()
        previous_count = -1
        time.sleep(check_interval_seconds)
        while True:
            new_count = self.get_number_of_indexed_events(
...
class SplunkContainer(SplunkInstance):
    def __init__(
        ...
        super().__init__(
            config, shared_test_objects, web_port, hec_port, management_port
        )
        self.ports = {
            "8000/tcp": web_port,
            "8088/tcp": hec_port,
        ...
        self.files_to_copy_to_instance["DATAMODELS"] = {
            "local_file_path": os.path.join(
                os.path.dirname(__file__), "../", "datamodels.conf.tar"
            ),
        ...
        self.files_to_copy_to_instance["AUTHORIZATIONS"] = {
            "local_file_path": os.path.join(
                os.path.dirname(__file__), "../", "authorize.conf.tar"
            ),
            "container_file_path": "/opt/splunk/etc/system/local",
        }

        self.mounts = [
            docker.types.Mount(
                source=os.path.abspath(os.path.join(pathlib.Path("."), "apps")),
                target="/tmp/apps",
                type="bind",
                read_only=True,
            )
    ...
    def prepare_apps_path(self) -> tuple[str, bool]:
        ...
        for app in self.config.apps:
            ...
            if app.local_path is not None:
                ...
                raise (
                    Exception(
                        f"No local, http, or Splunkbase path found for app {app.title}"
                    )
                )
    ...
    def stopContainer(self, timeout=10) -> bool:
        try:
            ...
            container: docker.models.containers.Container = (
                self.get_client().containers.get(self.get_name())
            )
    ...
    def setup(self):
        ...
        self.print(
            f"Starting container and installing [{len(self.config.apps)}] apps/TAs..."
        )
